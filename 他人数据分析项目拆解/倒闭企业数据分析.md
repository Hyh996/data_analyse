数据分析项目（倒闭企业数据分析）
项目链接：
https://github.com/TurboWay/bigdata_analyse/blob/main/DeathCompany/%E5%80%92%E9%97%AD%E4%BC%81%E4%B8%9A%E6%95%B0%E6%8D%AE%E5%88%86%E6%9E%90.ipynb
该文档是基于项目文件倒闭企业数据分析.ipynb结合一定个人理解所进行的内化整理
...
data = pd.read_csv('com.csv')
data.head()
注意c.render_notebook()是用于在 Jupyter Notebook 中渲染图表和数据的语句，以下不在赘述。当然修改为例如c.render('regional_distribution.html')的语句可以输出HTML格式的结果。以下分析是按照改成输出HTML格式来结合分析的
